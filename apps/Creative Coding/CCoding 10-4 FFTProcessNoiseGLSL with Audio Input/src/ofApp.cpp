...
void ofApp::setup(){
    ...
    gui.setup();
    gui.add(level.setup("Input Level", 0.5, 0, 2.0));
    gui.add(noiseFrequency.setup("Noise Frequency", 0.05, 0.0, 0.1));
    gui.add(noiseSpeed.setup("Noise Speed", 0.5, 0.0, 2.0));
    gui.loadFromFile("settings.xml");
    showGui = true;
    //サウンドの入力
//    soundStream.printDeviceList(); //Macで使用するオーディオ出力機器のデバイス番号リスト
...
void ofApp::update(){
    ...
    float lowValue = ofMap(fft.getSuperLowVal(), 0, 1, 0, level);
